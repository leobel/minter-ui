...
const MintToken = () => {
    ...
    const { wallet, txInfo, setTxInfo, mintToken } = useMint();
    ...
    const onMintToken = async () => {
        const tx = await mintToken(script, tokens);
        setTxInfo({ tx });
    }
    ...
    return (
        <>
            ...
            <button className='btn btn-primary mt-4 mb-4' onClick={onMintToken} disabled={!wallet || (tokens.length == 0 || tokens.some(t => (!t.address || !t.token))) || !script}>BuildTx</button>
