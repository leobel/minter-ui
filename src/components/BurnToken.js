...
const BurnToken = () => {
    ...
    const { wallet, txInfo, setTxInfo, burnToken } = useMint();

    const [script, setScript] = useState(null);
    const [tokens, setTokens] = useState([{ asset_name: '' }]);
    ...
    const onScriptSelected = (data) => {
        const { policy_id, policy, signers, reference_address, mint, reference } = JSON.parse(data);
        const _script = { policy_id, policy, signers, reference_address, mint, reference };
        setScript(_script);
    }
    ...
    const addToken = () => {
        setTokens(tokens.concat([{ asset_name: '' }]));
    }
    ...
    const onUpdateToken = async () => {
        const tx = await burnToken(script, tokens);
        setTxInfo({ tx });
    }
    ...
    return (
        <>
            ...
            <button className='btn btn-primary mt-4 mb-4' onClick={onUpdateToken} disabled={!wallet || (tokens.length == 0 || tokens.some(t => (!t.asset_name))) || !script}>BuildTx</button>
